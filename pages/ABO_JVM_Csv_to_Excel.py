...
if uploaded_file:
    ...
    # Affichage des abonnements annulés dans un tableau
    st.subheader("Liste des abonnements annulés (CANCELLED) :")
    cancelled_table = cancelled[['ID', 'Customer name', 'Customer email', 'Next order date']]

    # Tableau interactif avec des checkboxes pour chaque ligne
    st.write("Sélectionnez les abonnements annulés à inclure dans le fichier final :")
    selected_rows = []
    for i, row in cancelled_table.iterrows():
        checkbox_label = f"ID: {row['ID']} | Nom: {row['Customer name']} | Email: {row['Customer email']} | Next Order Date: {row['Next order date']}"
        if st.checkbox(checkbox_label, key=f"cancelled_{i}"):
            selected_rows.append(row)

    # Créer un DataFrame avec les lignes sélectionnées
    selected_cancelled_df = pd.DataFrame(selected_rows)
    ...
    # Afficher les abonnements annulés sélectionnés
    if not selected_cancelled_df.empty:
        st.subheader("Abonnements annulés sélectionnés :")
        st.dataframe(selected_cancelled_df)
    else:
        st.info("Aucun abonnement annulé sélectionné.")

    # Combiner les actifs et les annulés sélectionnés
    final_df = pd.concat([active, selected_cancelled_df])
